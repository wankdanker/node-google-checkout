				...
				'#' : {
					'results' : {
						'result' : map(data.addresses, function (_, address) {
							...
							//totalTax could be 0, so we can't just check if it's falsey.
							//check to see if the address object has a key named totalTax
							if (address.hasOwnProperty('totalTax')) {
								resultDoc['#']['total-tax'] = address.totalTax;
							}
